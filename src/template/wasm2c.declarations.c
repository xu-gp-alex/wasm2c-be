...
static inline f32 wasm_bswap_f32(f32 x) {
  u32 as_int;
  // 1. Interpret bits as integer
  memcpy(&as_int, &x, sizeof(f32));
  
  // 2. Perform the swap
  as_int = bswap_32(as_int);
  
  // 3. Interpret bits back as float
  f32 result;
...
static inline f64 wasm_bswap_f64(f64 x) {
  ...
  #define WABT_BSWAP_u8(v)   (v)             // Single byte: never swap
  #define WABT_BSWAP_s8(v)   (v)             // Single byte: never swap
  ...
  #define WABT_BSWAP_f32(v)  wasm_bswap_f32(v) // helper needed for float bitcast
  #define WABT_BSWAP_f64(v)  wasm_bswap_f64(v)
#else
  // Little Endian (Standard Wasm) -> Do nothing (Identity)
