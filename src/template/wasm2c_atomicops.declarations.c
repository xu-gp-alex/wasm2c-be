...
#define DEFINE_ATOMIC_RMW(name, opname, op, t1, t2)                          \
  static inline t2 name##_unchecked(wasm_rt_memory_t* mem, u64 addr,         \
    ...
    t1 host_ret = WABT_BSWAP_##t1(ret);                                      \
    t1 le_result = WABT_BSWAP_##t1(host_ret op wrapped);                     \
    wasm_rt_memcpy(MEM_ADDR(mem, addr, sizeof(t1)), &le_result, sizeof(t1)); \
    return (t2)host_ret;                                                     \
  }                                                                          \
  ...
  static inline t2 name##_shared_unchecked(wasm_rt_shared_memory_t* mem,     \
    ...
    ATOMIC_ALIGNMENT_CHECK(addr, t1);                                        \
    t1 wrapped = (t1)value;                                                  \
    _Atomic volatile t1* ptr =                                               \
        (_Atomic volatile t1*)MEM_ADDR(mem, addr, sizeof(t1));               \
    t1 expected = atomic_load(ptr);                                          \
    t1 desired;                                                              \
    t1 host_expected;                                                        \
    do {                                                                     \
      host_expected = WABT_BSWAP_##t1(expected);                             \
      desired = WABT_BSWAP_##t1(host_expected op wrapped);                   \
    } while (!atomic_compare_exchange_weak(ptr, &expected, desired));        \
    return (t2)host_expected;                                                \
  }                                                                          \
...
#define DEFINE_ATOMIC_XCHG(name, opname, t1, t2)                             \
  static inline t2 name##_unchecked(wasm_rt_memory_t* mem, u64 addr,         \
                                    t2 value) {                              \
    ATOMIC_ALIGNMENT_CHECK(addr, t1);                                        \
    t1 wrapped = WABT_BSWAP_##t1((t1)value);                                 \
    t1 ret;                                                                  \
    wasm_rt_memcpy(&ret, MEM_ADDR(mem, addr, sizeof(t1)), sizeof(t1));       \
    wasm_rt_memcpy(MEM_ADDR(mem, addr, sizeof(t1)), &wrapped, sizeof(t1));   \
    return (t2)WABT_BSWAP_##t1(ret);                                         \
  }                                                                          \
  DEF_MEM_CHECKS1(name, _, t1, return, t2, t2)                               \
  static inline t2 name##_shared_unchecked(wasm_rt_shared_memory_t* mem,     \
                                           u64 addr, t2 value) {             \
...
#define DEFINE_ATOMIC_CMP_XCHG(name, t1, t2)                                 \
  static inline t1 name##_unchecked(wasm_rt_memory_t* mem, u64 addr,         \
    ...
    t2 expected_wrapped = WABT_BSWAP_##t2((t2)expected);                     \
    t2 replacement_wrapped = WABT_BSWAP_##t2((t2)replacement);               \
    t2 ret;                                                                  \
  ...
  static inline t1 name##_shared_unchecked(                                  \
    ...
    return (t1)WABT_BSWAP_##t2(expected_wrapped);                            \
  }                                                                          \
